...
df = pd.read_csv("data/repair_estimates.csv")
...
# Vendor-level stats joined back to every row
...
def update_all(cost_r, labor_r, line_r, acc_min, prior_min,
    ...
    if apply_dates:
        mask &= (
            df["est_recv_dte"].dt.date.ge(start_d) &
            df["est_recv_dte"].dt.date.le(end_d)
        )
    ...
    # ── Chart 4: Est. amount box plot by damage type ──────────────────────────
    ...
    for appr, color, name in [(True, EM_GREEN, "Approved"),
        ...
        sub = filt[filt["auto_approved"] == appr]
        if sub.empty:
            continue
