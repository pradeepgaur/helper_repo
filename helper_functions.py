...
INCIDENTS: List[Dict[str, Any]] = [
    {"id": "25437",    "sub_text": "GP10 · Brian Lentz",  "status": "pending_ai_review"},
...
_DETAIL_25437: dict[str, Any] = {
    ...
    "photos": [
        ...
        {"url": "https://images.unsplash.com/photo-1622366582-8c14f44c65e8?w=400&h=560&fit=crop&auto=format",
         "lightbox_url": "https://images.unsplash.com/photo-1622366582-8c14f44c65e8?w=1200&h=800&fit=crop",
         "label": "License Plate", "badge": "plate", "orientation": "portrait"},
    ...
    "line_items": [
        {"line": "001",    "op": "RI",   "description": "Front bumper cover R&I",   "type": None, "part_num": None,         "price": None,     "qty": None, "labor": "1.1 B", "paint": None,     "ai_status": "approved",  "flag_special": False},
        {"line": "002",    "op": "RR",   "description": "Grille, Frt Bmpr Cvr",     "type": "N",  "part_num": "622546LY0A", "price": "$612.49","qty": "1",  "labor": "0.5 B", "paint": None,     "ai_status": "flagged",   "flag_special": False},
        {"line": "003 *",  "op": "PRPR", "description": "Panel, Rocker LT",         "type": None, "part_num": None,         "price": None,     "qty": None, "labor": "5 B",   "paint": None,     "ai_status": "approved",  "flag_special": False},
        {"line": "004",    "op": "REF",  "description": "Panel, Rocker LT",         "type": None, "part_num": None,         "price": None,     "qty": None, "labor": None,    "paint": "3 R",    "ai_status": None,        "flag_special": False},
        {"line": "005",    "op": "REF",  "description": "Panel, Rocker LT",         "type": None, "part_num": None,         "price": None,     "qty": None, "labor": None,    "paint": "0.3 R",  "ai_status": "flagged",   "flag_special": False},
        {"line": "006",    "op": "RR",   "description": "Tape, Quarter Panel LT",   "type": "N",  "part_num": "788656LB0A", "price": "$26.53", "qty": "1",  "labor": "0.2 B", "paint": None,     "ai_status": "approved",  "flag_special": False},
        {"line": "007",    "op": "RI",   "description": "Guard, Mud LT",            "type": None, "part_num": None,         "price": None,     "qty": None, "labor": "0.1 B", "paint": None,     "ai_status": None,        "flag_special": False},
        {"line": "008 *",  "op": "SUB",  "description": "Corrosion protection",     "type": None, "part_num": None,         "price": "$10.00", "qty": None, "labor": None,    "paint": "0 R",    "ai_status": "approved",  "flag_special": False},
        {"line": "009 *",  "op": "REF",  "description": "Color tint",               "type": None, "part_num": None,         "price": None,     "qty": None, "labor": None,    "paint": "0.5 R",  "ai_status": None,        "flag_special": False},
        {"line": "010 *",  "op": "SUB",  "description": "Cover car exterior",       "type": None, "part_num": None,         "price": "$5.00",  "qty": None, "labor": None,    "paint": "0 R",    "ai_status": "flagged",   "flag_special": False},
        {"line": "011 *",  "op": "SUB",  "description": "Hazardous waste removal",  "type": None, "part_num": None,         "price": "$5.00",  "qty": None, "labor": "0 B",   "paint": None,     "ai_status": "approved",  "flag_special": False},
        {"line": "012 *",  "op": "SUB",  "description": "Post-scan",                "type": None, "part_num": None,         "price": "$70.00", "qty": None, "labor": None,    "paint": "0.5 M",  "ai_status": None,        "flag_special": False},
        {"line": "013 * †","op": "SUB",  "description": "Front radar calibration",  "type": None, "part_num": None,         "price": "$760.00","qty": None, "labor": "0 B",   "paint": None,     "ai_status": "flagged",   "flag_special": True},
    ],
...
def compute_progress_tabs(detail: Dict[str, Any]) -> List[Dict[str, str]]:
    """Derive progress-tab statuses from the actual incident data."""
    ...
    # Tab 2 — Parts rate & discount: line items that have ai_status
    li_statuses = [li.get("ai_status") for li in detail["line_items"] if li.get("type") == "N" or li.get("price")]
    parts_status = _tab_status(li_statuses)

    # Tab 3 — Labor rate & discount: breakdown.labor items
    labor_status = _tab_status([i["ai_status"] for i in bd["labor"]["items"]])

    # Tab 4 — Materials check: breakdown.materials items
    mat_status = _tab_status([i["ai_status"] for i in bd["materials"]["items"]])

    # Tab 5 — Totals & discount matching: parts + miscellaneous breakdown
    totals_vals = [i["ai_status"] for i in bd["parts"]["items"] + bd["miscellaneous"]["items"]]
    totals_status = _tab_status(totals_vals) if any(totals_vals) else "inactive"

    return [
        {"label": "Vehicle ID check",          "status": id_status},
        {"label": "Parts rate & discount",      "status": parts_status},
        {"label": "Labor rate & discount",      "status": labor_status},
        {"label": "Materials check",            "status": mat_status},
        {"label": "Totals & discount matching", "status": totals_status},
    ]
...
def _make_detail(
    ...
    detail: Dict[str, Any] = {
        ...
        "line_items": [
            {"line": "001", "op": "RR",  "description": "Bumper cover, front",    "type": "N",  "part_num": "521190R060", "price": "$289.95","qty": "1",  "labor": "2.0 B","paint": None,    "ai_status": "approved","flag_special": False},
            {"line": "002", "op": "REF", "description": "Bumper cover, front",    "type": None, "part_num": None,         "price": None,    "qty": None, "labor": None,   "paint": "2.0 R", "ai_status": "approved","flag_special": False},
            {"line": "003", "op": "RR",  "description": "Headlamp assembly, LT", "type": "N",  "part_num": "811500R020", "price": "$542.00","qty": "1",  "labor": "0.8 B","paint": None,    "ai_status": "flagged", "flag_special": False},
            {"line": "004 *","op": "SUB","description": "Post-scan",              "type": None, "part_num": None,         "price": "$70.00","qty": None, "labor": None,   "paint": "0.5 M", "ai_status": "approved","flag_special": False},
            {"line": "005 *","op": "SUB","description": "Hazardous waste removal","type": None, "part_num": None,         "price": "$5.00", "qty": None, "labor": "0 B",  "paint": None,    "ai_status": "approved","flag_special": False},
        ],
        ...
        "breakdown": {
            "labor": {
                "total": labor_total,
                "items": [
                    {"label": f"Body (2.8 hrs @ {body_rate.split('/')[0].strip()})",    "value": "$112.00","value_per_ai": "$112.00","ai_status": "approved","negative": False},
                    {"label": f"Refinish (2.0 hrs @ {body_rate.split('/')[0].strip()})","value": "$80.00", "value_per_ai": "$80.00", "ai_status": "approved","negative": False},
                ],
            },
            "parts": {
                "total": parts_total,
                "items": [
                    {"label": "New parts subtotal","value": "$831.95","value_per_ai": "$798.40","ai_status": "flagged","negative": False},
                    {"label": "Adjustment –15%",  "value": "–$124.80","value_per_ai": "–$124.80","ai_status": "approved","negative": True},
                ],
            },
            "materials": {
                "total": "$88.00",
                "items": [
                    {"label": "Paint materials","value": "$88.00","value_per_ai": "$88.00","ai_status": "approved","negative": False},
                ],
            },
            "miscellaneous": {
                "total": "$75.00",
                "items": [
                    {"label": "Other – sublet","value": "$75.00","value_per_ai": "$75.00","ai_status": "approved","negative": False},
                ],
            },
        },
        "total": {
    ...
    # Inject computed progress tabs
    detail["progress_tabs"] = compute_progress_tabs(detail)
    return detail
...
def get_incident_detail(incident_id: str) -> Optional[Dict[str, Any]]:
    ...
    if detail is None:
        return None
    # Always recompute progress tabs from live data (covers _DETAIL_25437 too)
    detail = dict(detail)
    detail["progress_tabs"] = compute_progress_tabs(detail)
    vi = detail["vehicle_info"]
    detail["vehicle_info"]["ai_verified"] = all(
