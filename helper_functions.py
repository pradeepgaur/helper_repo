...
risk_latest AS (
    SELECT est_id, rvsn_nbr, est_hist_amt
    FROM risk_ranked
    WHERE rn_desc = 1   -- latest record per (est_id, rvsn_nbr)
),
...
risk_pivoted AS (
    SELECT
        ...
        MAX(est_hist_amt) FILTER (
            WHERE rvsn_nbr = (
                SELECT MAX(r2.rvsn_nbr)
                FROM risk_latest r2
                WHERE r2.est_id = l.est_id
            )
        )                                                   AS latest_amt
    FROM risk_latest l
    GROUP BY est_id
