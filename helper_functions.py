        ...
        html.Div([
            ...
            html.Div(className="charts-grid", children=[
                ...
                html.Div(className="chart-panel", children=[
                    ...
                    dcc.Loading(type="dot", color=EM_GREEN, children=
...
def update_all(cost_r, labor_r, line_r, acc_min, prior_min,
    ...
    n_rev_gt1      = int(kpis.get("n_rev_gt1",         0) or 0)
    time_saved_hrs  = int(round(float(kpis.get("time_saved_hrs",  0) or 0),0))
    time_saved_days = int(round(float(kpis.get("time_saved_days", 0) or 0),0) )
    mean_correct   = float(kpis.get("mean_correct_amt",   0) or 0)
    mean_wrong     = float(kpis.get("mean_wrong_amt",    0) or 0)
    estimated_risk = float(kpis.get("estimated_risk",    0) or 0)

    # Derived KPIs
    coverage    = n / n_date if n_date > 0 else 0.0
    precision   = n_rev1 / n if n > 0 else 0.0
    rental_days = time_saved_days
    rate        = float(rate_per_day) if rate_per_day else 50.0
    biz_value   = rental_days * rate
    est_risk    = estimated_risk
    ...
    return (
        ...
        f"{time_saved_hrs:,.0f} hrs", f"sum of approval time for {n_rev1:,} first-revision estimates",
        f"{rental_days:,.0f} days",   f"days between estimate approved - received date",
        f"${biz_value:,.0f}",         f"{rental_days:,.1f} days × ${rate:,.0f}/day",
        f"${est_risk:,.0f}",          f"sum of (latest revision amt − rev1 amt) for {n_rev_gt1:,} revised estimates",
        date_err,
