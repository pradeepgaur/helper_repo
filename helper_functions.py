...
RETURNS TEXT LANGUAGE sql IMMUTABLE AS $$
    WITH base AS (
        SELECT
            LOWER(TRIM(SPLIT_PART(p_dsc, ':', 1))) AS prefix,
            LOWER(p_dsc)                            AS full_text
    )
    SELECT CASE
        -- Hail detection from FULL DESCRIPTION
        WHEN full_text ILIKE '%hail%'                   THEN 'Hail'

        -- Bumpers
        WHEN prefix ILIKE '%bumper%'                    THEN 'Bumper'

        -- Fender
        WHEN prefix ILIKE '%fender%'                    THEN 'Fender'

        -- Doors
        WHEN prefix ILIKE '%door%'                      THEN 'Door'

        -- Glass / Windows
        WHEN prefix ILIKE '%window%'
          OR prefix ILIKE '%windshield%'                THEN 'Glass'

        -- Lights
        WHEN prefix ILIKE '%light%'                     THEN 'Lights'

        -- Panels / Quarter Panels / Side Panels
        WHEN prefix ILIKE '%panel%'
          OR prefix ILIKE '%quarter%'                   THEN 'Panels'

        -- Roof / Hood / Decklid / Trunk
        WHEN prefix ILIKE '%roof%'
          OR prefix ILIKE '%hood%'
          OR prefix ILIKE '%decklid%'
          OR prefix ILIKE '%tailgate%'
          OR prefix ILIKE '%trunk%'                     THEN 'Body Major'

        -- Wheels / Tires
        WHEN prefix ILIKE '%wheel%'
          OR prefix ILIKE '%tire%'                      THEN 'Wheels/Tires'

        -- Mirrors
        WHEN prefix ILIKE '%mirror%'                    THEN 'Mirrors'

        -- Interior removed — falls through to Others
...
-- ── 4a. Risk — latest est_hist_amt per revision from ice.t_6mo_est_hist ───────
--         Only for filtered estimates that have rvsn_nbr > 1

-- Step 1: pre-compute distinct revised est_ids once — cast done here only
risk_ids AS (
    SELECT DISTINCT est_id::NUMERIC AS est_id
    FROM filtered
    WHERE rvsn_nbr > 1
),

-- Step 2: INNER JOIN instead of IN subquery — avoids per-row subquery re-eval
--         and lets the planner use an index on ice.t_6mo_est_hist.est_id
risk_ranked AS (
    SELECT
        h.est_id,
        h.rvsn_nbr,
        h.est_hist_amt,
        ROW_NUMBER() OVER (
            PARTITION BY h.est_id, h.rvsn_nbr
            ORDER BY h.last_updtd_tmstp DESC
        ) AS rn_desc
    FROM ice.t_6mo_est_hist h
    INNER JOIN risk_ids r ON h.est_id = r.est_id
),

-- Step 3: keep latest record per (est_id, rvsn_nbr)
--         add max_rvsn_nbr as window function — eliminates correlated subquery below
risk_latest AS (
    SELECT
        est_id,
        rvsn_nbr,
        est_hist_amt,
        MAX(rvsn_nbr) OVER (PARTITION BY est_id) AS max_rvsn_nbr
    FROM risk_ranked
    WHERE rn_desc = 1
),

-- Step 4: pivot — no correlated subquery, uses pre-computed max_rvsn_nbr
risk_pivoted AS (
    SELECT
        est_id,
        MAX(CASE WHEN rvsn_nbr = 1            THEN est_hist_amt END) AS rvsn1_amt,
        MAX(CASE WHEN rvsn_nbr = max_rvsn_nbr THEN est_hist_amt END) AS latest_amt
    FROM risk_latest
    GROUP BY est_id
),
...
kpis AS (
    SELECT
        ...
        (SELECT COALESCE(SUM(latest_amt - rvsn1_amt), 0)
         FROM risk_pivoted
         WHERE (latest_amt - rvsn1_amt) < 0)                                 AS estimated_risk
    FROM filtered
),
