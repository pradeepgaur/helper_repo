...
def update_all(cost_r, labor_r, line_r, acc_min, prior_min,
    ...
    est_risk    = (mean_wrong - mean_correct) * n_rev_gt1
    ...
    return (
        ...
        f"{rental_days:,.0f} days",   f"days between (apprv_dte − est_recv_dte) for {n_rev1:,} first-revision estimates",
        f"${biz_value:,.0f}",         f"{rental_days:,.1f} days × ${rate:,.0f}/day",
        f"${est_risk:,.0f}",          f"(mean rev1 ${mean_correct:,.0f} − mean rev>1 ${mean_wrong:,.0f}) × {n_rev_gt1:,}",
        date_err,
