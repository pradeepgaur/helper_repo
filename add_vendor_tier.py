...
for depth in [3, 4, 5]:
    clf = DecisionTreeClassifier(
        max_depth=depth,
        min_samples_leaf=150,   # ~0.5% of data — avoids tiny overfitted leaves
...
def extract_rules(tree, feature_names):
    ...
    def recurse(node, conditions):
        if tree_.feature[node] == _tree.TREE_UNDEFINED:
            ...
            prediction = 1 if approval_r >= 0.5 else 0
            rules.append({
            ...
            recurse(node * 2 + 1, conditions + [f"{feat} <= {thresh}"])
            recurse(node * 2 + 2, conditions + [f"{feat} >  {thresh}"])
...
model_df["dt_prediction"]   = clf.predict(full_X)
